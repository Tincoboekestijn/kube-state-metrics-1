			...
			wrapPodFunc(func(p *v1.Pod) *metric.Family {
				...
				for _, c := range p.Spec.Containers {
					...
					for resourceName, val := range req {
						...
						case v1.ResourceCPU:
							ms = append(ms, &metric.Metric{
								LabelValues: []string{p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], p.Labels["spark-app-selector"], p.Labels["spark-app-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitCore)},
								Value:       float64(val.MilliValue()) / 1000,
							})
						case v1.ResourceStorage:
							fallthrough
						case v1.ResourceEphemeralStorage:
							ms = append(ms, &metric.Metric{
								LabelValues: []string{p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], p.Labels["spark-app-selector"], p.Labels["spark-app-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitByte)},
								Value:       float64(val.Value()),
							})
						case v1.ResourceMemory:
							ms = append(ms, &metric.Metric{
						...
						default:
							...
							if isExtendedResourceName(resourceName) {
								ms = append(ms, &metric.Metric{
									LabelValues: []string{p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], p.Labels["spark-app-selector"], p.Labels["spark-app-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitInteger)},
									Value:       float64(val.Value()),
								})
							}
						}
				...
				for _, metric := range ms {
					metric.LabelKeys = []string{"node", "volcano_namespace", "scheduler", "volcano_job", "queue", "spark_app_id", "spark_app_name", "resource", "unit"}
				}
