	...
	return []generator.FamilyGenerator{
		...
		*generator.NewFamilyGenerator(
			...
			wrapPodFunc(func(p *v1.Pod) *metric.Family {
				...
				for i, cs := range p.Status.ContainerStatuses {
					ms[i] = &metric.Metric{
						LabelKeys:   []string{"container", "job_name", "queue", "scheduler"},
						LabelValues: []string{cs.Name, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], p.Spec.SchedulerName},
						Value:       boolFloat64(cs.State.Running != nil),
					}
				}
		...
		*generator.NewFamilyGenerator(
			"kube_pod_volcano_container_resource_used",
			"The number of requested request resource by a container.",
			metric.Gauge,
			"",
			wrapPodFunc(func(p *v1.Pod) *metric.Family {
				...
				for _, c := range p.Spec.Containers {
					...
					for resourceName, val := range req {
						...
						case v1.ResourceCPU:
							ms = append(ms, &metric.Metric{
								LabelValues: []string{c.Name, p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitCore)},
								Value:       float64(val.MilliValue()) / 1000,
							})
						case v1.ResourceStorage:
							fallthrough
						...
						case v1.ResourceMemory:
							ms = append(ms, &metric.Metric{
								LabelValues: []string{c.Name, p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitByte)},
								Value:       float64(val.Value()),
							})
						default:
							if isHugePageResourceName(resourceName) {
							...
							if isExtendedResourceName(resourceName) {
								ms = append(ms, &metric.Metric{
									LabelValues: []string{c.Name, p.Spec.NodeName, p.Namespace, p.Spec.SchedulerName, p.Labels["volcano.sh/job-name"], p.Labels["volcano.sh/queue-name"], sanitizeLabelName(string(resourceName)), string(constant.UnitInteger)},
									Value:       float64(val.Value()),
								})
							}
						}
				...
				for _, metric := range ms {
					metric.LabelKeys = []string{"container", "node", "volcano_namespace", "scheduler", "volcano_job", "queue", "resource", "unit"}
				}
